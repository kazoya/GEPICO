...
import { ClipboardList, LayoutDashboard, MessageCircle, MoreHorizontal } from "lucide-react";
import { cn } from "@/lib/utils";

const items = [
  { href: "/", label: "الرئيسية", icon: LayoutDashboard, match: (p: string) => p === "/" },
  { href: "/dashboard", label: "القيادة", icon: LayoutDashboard, match: (p: string) => p === "/dashboard" },
      ...
      <ul className="grid grid-cols-5 gap-1">
        ...
        <li>
          <button
            type="button"
            onClick={onMore}
            className="flex w-full cursor-pointer flex-col items-center gap-0.5 rounded-lg py-1.5 text-[11px] text-muted-foreground"
          >
