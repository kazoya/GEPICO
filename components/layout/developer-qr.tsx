...
export function DeveloperQr({ compact = false }: DeveloperQrProps) {
  ...
  return (
    <a
      ...
      rel="noreferrer"
      className={cn(
        "group relative flex cursor-pointer items-center gap-3 rounded-xl border border-gold/50 bg-primary px-2.5 py-2 text-primary-foreground shadow-[0_0_24px_rgb(4_120_87_/_0.18)] outline-none transition-shadow hover:shadow-[0_0_28px_rgb(4_120_87_/_0.32)] focus-visible:ring-2 focus-visible:ring-gold/60",
      )}
    ...
    >
      ...
      <span className={cn("min-w-0 text-right leading-snug", compact && "sr-only")}>
        ...
        <span className="mt-0.5 block text-[12px] text-primary-foreground/90">امسح الرمز</span>
        <span className="mt-0.5 hidden font-mono text-[10px] text-primary-foreground/65 sm:block" dir="ltr">
          {phone}
        </span>
        <span className="sr-only">{siteConfig.developer.nameAr} — واتساب</span>
      </span>
    </a>
  );
}
